// source: https://github.com/DCtheTall/glsl-canny-edge-detection/tree/master
...
uniform ivec2 res;
uniform float weakThreshold;
uniform float strongThreshold;
...
void main() {
    // canny:
    float edge = cannyEdgeDetection(tex, uv, vec2(res), weakThreshold, strongThreshold);

    // sobel:
    // vec2 grad = getTextureIntensityGradient(tex, uv, vec2(res));
    // float edge = length(grad);
   
    out_edge = vec4(edge,edge,edge,1);
}